...
import sys
# 获取当前脚本（fasterwhisper_asr.py）的绝对路径
current_script_path = os.path.abspath(__file__)
# 向上两级找到项目根目录（GPT-SoVITS）
...
def execute_asr(input_folder, output_folder, model_path, language, precision):
    ...
    # # 原代码（可能是这样）
    # device = "cuda" if torch.cuda.is_available() else "cpu"
    # precision = "float16"  # 原默认值

    # 改为：适配 Apple Silicon（M4 芯片）
    # device = "mps" if torch.backends.mps.is_available() else "cpu"  # M 芯片用 MPS 加速
    # if device == "mps":
    #     precision = "bfloat16"  # MPS 支持 bfloat16，速度比 float32 快，精度接近 float16
    # elif device == "cuda":
    #     precision = "float16"  # 有 GPU 时用 float16
    # else:
    #     precision = "float32"  # 纯 CPU  fallback
    device = "cpu"
    precision = "float32"

    # 保持这行不变，使用修改后的 device 和 precision
